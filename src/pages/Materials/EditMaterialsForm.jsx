...
export default function EditMaterialsForm({id}) {
    ...
    const onFinish = async (values) => {
        try {
            const response = await updateMaterial(id, values);
            messageApi.success(" 🔥 Material updated successfully!");
            setTimeout(()=>
                dispatch(setEditActive(false)),1000);

        } catch (error) {
            console.error("Error Updating Material:", error.response ? error.response.data : error);
