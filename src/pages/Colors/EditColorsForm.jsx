...
export default function EditColorsForm({id}) {
    ...
    const onFinish = async (values) => {
        try {
            const response = await updateColor(id, values);
            messageApi.success("Color updated successfully!");
            setTimeout(()=>
                dispatch(setEditActive(false)),1000);

        } catch (error) {
            console.error("Error Updating Color:", error.response ? error.response.data : error);
            messageApi.error("Failed to update color.");
            setTimeout(()=>
                dispatch(setEditActive(false)),1000);
        }
    };
    ...
    useEffect(()=>{
        if (selectedColor) {
            form.setFieldsValue({
                ...
                rgb_code: selectedColor.rgb_code,
            });
        }
    },[selectedColor])
    ...
    return (
        <div className={style.container}>
            ...
            <Form form={form} layout="vertical" className={style.form} onFinish={onFinish}>
                ...
                <div className={style.row}>
                    <Form.Item label="HEX Codi" name="hex_code" className={style.input}>
                        <Input placeholder="Enter hex code" />
                    </Form.Item>
                    <Form.Item label="RGB Codi" name="rgb_code" className={style.input}>
                        <Input placeholder="Enter rgb code" />
                    </Form.Item>
                </div>
